...
function getApyTreeTop($menu_id){
	...
	foreach($top_category_foreach as $cat_id => $cat_value){
				if((get_class($cat_value) != 'CMS_Catalog')&&(get_class($cat_value) != 'CMS_Attachment')&&(get_class($cat_value) != 'CMS_EventCalendar_Event'))
...
//  	       			$title = "<DIV style='margin-top:10px;'><img src='mediafiles/$image'></DIV>";
				...
				if (get_class($cat_value) == 'CMS_EventCalendar_Event'){
				...
				if (get_class($cat_value) == 'CMS_Gallery'){
				...
				if (get_class($cat_value) == 'CMS_Catalog'){
