	...
	<tr>
		<td colspan="3">
			<table class="tb_title">
				<tr>
					<td><img class="img_middle" src="images/language_view.png" alt="" />&nbsp;&nbsp;{insert name='tr' value='Manažér jazykov / Nový jazyk'}</td>
				</tr>
			</table>
		</td>
	</tr>
		...
		<td class="td_middle_center">
			<table class="tb_list">
				...
				<tr>
					...
					<td width="85%">{insert name='language_enable' input='new' assign='language_enable'}
						<select name="code">

						{foreach name='lang' from=$lang_list_iso item='lang_id'}
							{insert name="language_search" input = $language_enable select = $lang_id.iso_639_1_code assign='showLanguage'}
							{if ($lang_id.iso_639_1_code neq "") AND ($showLanguage)}
							<option value="{$lang_id.iso_639_1_code}" {if $lang_id.iso_639_1_code eq "sk"}selected{/if} >{$lang_id.name}</option>
							{/if}
						{/foreach}
						</select>
					</td>
