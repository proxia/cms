...
{assign var='title_product' value = $detail_product->getTitle() }
{php}
		...
		<td class="td_middle_center">
			<table class="tb_list2">
				...
				<tr class="tr_header">
					...
					<td class="td_align_center">{insert name='tr' value='Hodnota atribútu pre produkt'} - {$detail_product->getTitle()}</td>
					<td class="td_align_center">{insert name='tr' value='ID atribútu'}</td>
				</tr>
				...
				<input type="hidden" id="product_id" name="product_id" value="{$detail_product->getId()}" />
				<input type="hidden" id="start" name="start" value="{$start}" />
			{foreach name='attribut' from=$attribut_list item='attribut_id' }
				<input type="hidden" name="map_id[{$attribut_id->getAttributeDefinitionId()}]" value="{$attribut_id->getId()}" />
				...
				<tr id="row{$smarty.foreach.attribut.iteration}" onmouseover="pozadieIN('form1','{$smarty.foreach.attribut.iteration}')" onmouseout="pozadieOUT('form1','{$smarty.foreach.attribut.iteration}')">
					<td class="td_align_center">{$smarty.foreach.attribut.iteration}</td>
					<td><input onclick="setRowBgColor('form1',{$smarty.foreach.attribut.iteration})" id="box{$smarty.foreach.attribut.iteration}" type="checkbox" name="row_id[]" value="{$attribut_id->getAttributeDefinitionId()}" /></td>
					<td>
						{assign var='attribut_name' value=$attribut_id->getAttributeDefinitionId()}
						<a href="./?module=CMS_Catalog&mcmd=9&row_id[]={$attribut_id->getAttributeDefinitionId()}">
					...
					<td class="td_align_center">
						{insert name=getAttributValue productId=$detail_product->getId() attributId=$attribut_id->getAttributeDefinitionId()}
					</td>
					<td class="td_align_center">{$attribut_id->getAttributeDefinitionId()}</td>
				</tr>
			{/foreach}
				</form>
			</table>
