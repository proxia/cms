...
try{
	require_once($GLOBALS['smarty']->get_template_vars('path_relative')."themes/default/scripts/functions.php");
	...
	switch ($_GET["mcmd"]) {
		...
		case "1":{
			...
			$limit = $_GET['setPerPage'] && $_GET['setPerPage'] != 'null'? $_GET['setPerPage'] : $GLOBALS["perpage"];
		...
		case "5":{
			if(isset($_REQUEST['row_id'][0]))
			{
				...
				foreach ( $items as $item )
				{
						...
						$sort_item->thumbnail = "/admin/img.php?path={$path}&w=80";
		...
		case "999":{
			...
			if(!$hasFtp){
				$path = "..".$GLOBALS['project_folder']."/".$_SESSION['user']['name']."/mediafiles/".$_GET['m_directory'];
				if (!strpos(realpath($path),'/mediafiles'))
				{
					$_GET['m_directory']='/';
				}
			}
			...
			if($hasFtp){
				$list = getDirectoryListFtp($directory);
			}else{
				$list = getDirectoryList($directory);
			}

			$GLOBALS["smarty"]->assign("media_list", $list);
