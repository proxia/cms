...
use cms_modules\cms_catalog\classes\CMS_Catalog_AttributeDefinitionList;
use cms_modules\cms_catalog\classes\CMS_Catalog_Branch;
use cms_modules\cms_catalog\classes\CMS_Catalog_ProductList;
use cms_modules\cms_gallery\classes\CMS_Gallery;
use cms_modules\cms_gallery\classes\CMS_GalleryList;
use cms_modules\cms_inquiry\classes\CMS_Inquiry;
use cms_modules\cms_inquiry\classes\CMS_Inquiry_Question;
use cms_modules\cms_inquiry\classes\CMS_InquiryList;
...
function createButton($class="tb_toolbar", $func="", $form_name="", $list_start=0, $list_end=0, $href="", $act_type="", $act_value="", $image="", $text="", $title=""){
	...
	// hack na newsletter
	//echo $form_name;exit;
	if ( ($_GET['module']== 'cms_modules\cms_newsletter\classes\CMS_Newsletter') && ($form_name == "name_form"))
			$fname = $form_name;
	elseif ($_GET['module']== 'cms_modules\cms_newsletter\classes\CMS_Newsletter')
		$fname = "'$form_name'";
...
function insert_getParentMenuList($input){
	if ($input['class'] != 'cms_modules\cms_event_calendar\classes\CMS_EventCalendar_Event'){
...
function insert_getObjectType($object){
	...
	elseif (get_class($object['item']) == 'cms_modules\cms_gallery\classes\CMS_Gallery'){
			if ($object['set'] == "number" )
					return 1;
	...
	elseif (get_class($object['item']) == 'cms_modules\cms_catalog\classes\CMS_Catalog'){
...
elseif (get_class($object['item']) == 'cms_modules\cms_event_calendar\classes\CMS_EventCalendar_Event'){
