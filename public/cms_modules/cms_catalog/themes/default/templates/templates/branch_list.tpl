		...
		<td class="td_middle_center">
			...
			{foreach name='branch' from=$branch_list item='branch_id'}
				...
				<tr id="row{$smarty.foreach.branch.iteration}" onmouseover="pozadieIN('form1','{$smarty.foreach.branch.iteration}')" onmouseout="pozadieOUT('form1','{$smarty.foreach.branch.iteration}')">
					...
					<td><input {if $showCheckbox eq 0}disabled="disabled"{/if} onclick="setRowBgColor('form1',{$smarty.foreach.branch.iteration})"  id="box{$smarty.foreach.branch.iteration}" type="checkbox" name="row_id[]" value="{$branch_id.object.id}" /></td>
					<td>
						{if $showItem eq 1}
   					...
   					<td class="td_align_center">
   						{if $numBranch > 0}
							{if $showItemEdit eq 1}
								<a href="./?module=CMS_Catalog&mcmd=27&&row_id[]={$branch_id.object.id}"><img src="{insert name='merge' value1=$path_relative value2='themes/default/images/all_m_restore.png'}" width="16"/></a>
							{else}
   						...
   						{if $showProduct eq 1 && $numBranch eq 0}
							{if $showItemEdit eq 1}
								<a href="./?module=CMS_Catalog&mcmd=28&&row_id[]={$branch_id.object.id}"><img src="{insert name='merge' value1=$path_relative value2='themes/default/images/all_m_restore.png'}" width="16"/></a>
							{else}
								<img src="{insert name='merge' value1=$path_relative value2='themes/default/images/all_m_restore.png'}" width="16" />
							{/if}
   						{/if}
					...
					<td>
						{if $numBranch eq 0}
							{if $showProduct eq 1}
								<a href="./?mcmd=10&module={$module}&s_category={$branch_id.object.id}"><img src="{insert name='merge' value1=$path_relative value2='themes/default/images/product_s.gif'}" /></a> ({$numProduct})
							{else}
								<img src="{insert name='merge' value1=$path_relative value2='themes/default/images/product_s.gif'}" /> ({$numProduct})
							{/if}
						{/if}
					</td>
					{if $numBranch eq 0}
