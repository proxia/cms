...
<table class="tb_middle">
	...
	<tr>
		...
		<td class="td_middle_center">
			...
			{foreach name='attribut' from=$attribut_list item='attribut_id' }
				<input type="hidden" name="map_id[{$attribut_id->getId()}]" value="{insert name=getMapId productId=$detail_product->getId() attributId=$attribut_id->getId()}" />
				...
				<tr id="row{$smarty.foreach.attribut.iteration}" onmouseover="pozadieIN('form1','{$smarty.foreach.attribut.iteration}')" onmouseout="pozadieOUT('form1','{$smarty.foreach.attribut.iteration}')">
					...
					<td><input onclick="setRowBgColor('form1',{$smarty.foreach.attribut.iteration})" id="box{$smarty.foreach.attribut.iteration}" type="checkbox" name="row_id[]" value="{$attribut_id->getId()}" /></td>
					<td>
					...
					<td class="td_align_center">
						<input type="text" name="attribut_value[{$attribut_id->getId()}]" size="30" value="{insert name=getAttributValue productId=$detail_product->getId() attributId=$attribut_id->getId()}">
					</td>
