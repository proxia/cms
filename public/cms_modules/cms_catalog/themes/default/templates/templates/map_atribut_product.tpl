...
{assign var='title_attrib' value = $detail_attribut->getTitle()}
{php}
		...
		<td class="td_middle_center">
			<table class="tb_list2">
				...
				<tr class="tr_header">
					...
					<td class="td_align_center">{insert name='tr' value='Hodnota atribútu'} - {$detail_attribut->getTitle()}</td>
				...
				<input type="hidden" id="attribut_id" name="attribut_id" value="{$detail_attribut->getId()}" />
			{foreach name='product' from=$product_list item='product_id'}
					{insert name=getMapId productId=$product_id->getId() attributId=$detail_attribut->getId() assign=mapId}
				<input type="hidden" name="map_id[{$product_id->getId()}]" value="{$mapId}" />
				...
				<tr id="row{$smarty.foreach.product.iteration}" onmouseover="pozadieIN('form1','{$smarty.foreach.product.iteration}')" onmouseout="pozadieOUT('form1','{$smarty.foreach.product.iteration}')">
					...
					<td class="td_align_center">
						<input type="text" name="attribut_value[{$product_id->getId()}]" size="30" value="{insert name=getAttributValue productId=$product_id->getId() attributId=$detail_attribut->getId()}">
					</td>
