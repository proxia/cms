<?php
	$is_help = 0;
	if (file_exists("themes/default/help/mcmd".$_GET['mcmd']."_company.html"))
			$is_help=1;
	echo"<table class=\"tb_toolbar\">";
	
		// company manager list
		if($_GET['mcmd'] == 1){
			if(($GLOBALS['user_login_type'] == CMS_UserLogin::ADMIN_USER) || ($GLOBALS['user_login']->checkPrivilege(106, CMS_Privileges::ADD_PRIVILEGE) === true))
			...
			if(($GLOBALS['user_login_type'] == CMS_UserLogin::ADMIN_USER) || ($GLOBALS['user_login']->checkPrivilege(106, CMS_Privileges::UPDATE_PRIVILEGE) === true)){
				...
				createButton('tb_toolbar','ischecked','form1',1,$GLOBALS['smarty']->get_template_vars(max_list),'#','c_isEnabled',1,'all_m_publish.png',tr('Ukáž'),tr('Ukáž'));
				createButton('tb_toolbar','ischecked','form1',1,$GLOBALS['smarty']->get_template_vars(max_list),'#','c_isEnabled',0,'all_m_unpublish.png',tr('Ukry'),tr('Ukry'));
			}
			else{
			...
			if(($GLOBALS['user_login_type'] == CMS_UserLogin::ADMIN_USER) || ($GLOBALS['user_login']->checkPrivilege(106, CMS_Privileges::DELETE_PRIVILEGE) === true))
			...
			if ($is_help)
				createButton('tb_toolbar','','','1','','themes/default/help/mcmd'.$_GET['mcmd'].'_company.html','cmd','help','all_m_help.png',tr('Pomocník'),tr('Pomocník'));
		}
		...
		if($_GET['mcmd'] == 2){
			...
			if ($is_help)
				createButton('tb_toolbar','','','1','','themes/default/help/mcmd'.$_REQUEST['mcmd'].'_company.html','cmd','help','all_m_help.png',tr('Pomocník'),tr('Pomocník'));
		}
