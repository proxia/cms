<?php
	...
	$is_help = 0;
	if (file_exists("themes/default/help/mcmd".$_GET['mcmd']."_gallery.html"))
			$is_help=1;
	echo"<table class=\"tb_toolbar\">";
		...
		if($_GET['mcmd'] == 1){
			...
			if(($GLOBALS['user_login_type'] == CMS_UserLogin::ADMIN_USER) || ($GLOBALS['user_login']->checkPrivilege(103, CMS_Privileges::ADD_PRIVILEGE) === true))
				tb_toolcreateButton('tb_toolbar','','','','','./?mcmd=2&module='.$GLOBALS['smarty']->get_template_vars('module').'','','','all_m_add.png',tr('Nová'),tr('Nová'));
			else
				createOffButton('all_m_add_off.png',tr('Nová'),tr('Nová'));

			if(($GLOBALS['user_login_type'] == CMS_UserLogin::ADMIN_USER) || ($GLOBALS['user_login']->checkPrivilege(103, CMS_Privileges::UPDATE_PRIVILEGE) === true))
			{
			...
			if(($GLOBALS['user_login_type'] == CMS_UserLogin::ADMIN_USER) || ($GLOBALS['user_login']->checkPrivilege(103, CMS_Privileges::DELETE_PRIVILEGE) === true))
				...
				if ($is_help)
					createButton('tb_toolbar','','','1','','themes/default/help/mcmd'.$_GET['mcmd'].'_gallery.html','cmd','help','all_m_help.png',tr('Pomocník'),tr('Pomocník'));
			}
		...
		if($_GET['mcmd'] == 2){
			...
			createButton('tb_toolbar','','','','','./?'.getReturnParameter().'','','','all_m_back.png',tr('Späť'),tr('Späť'));
				...
				if ($is_help)
					createButton('tb_toolbar','','','1','','themes/default/help/mcmd'.$_REQUEST['mcmd'].'_gallery.html','cmd','help','all_m_help.png',tr('Pomocník'),tr('Pomocník'));
			}
		...
		if($_GET['mcmd'] == 4){
			...
			if(($GLOBALS['user_login_type'] == CMS_UserLogin::ADMIN_USER) || ($GLOBALS['user_login']->checkPrivilege(103, CMS_Privileges::UPDATE_PRIVILEGE) === true))
			{
				createButton('tb_toolbar','checkControls','form1',0,0,'#','attach_update','1','all_m_save.png',tr('Uložiť'),tr('Uložiť'));
				//createButton('tb_toolbar','checkControls','form1',0,0,'#','go','edit','all_m_apply.png',tr('Použiť'),tr('Použiť'));
			}
			else
			{
