...
try{
	...
	if (is_Array($_POST['row_id'])){
		...
		if($_POST['go']==3){
			...
			$link = urlPost($ignore);
			Header("Location: ../../admin/?mcmd=3&module=CMS_BannerManager$link");
			exit;
		}
		...
		foreach ($_POST['row_id'] as $name => $id){
			$banner = $id === '0' ? new CMS_BannerManager_Banner() : new CMS_BannerManager_Banner($id);
			$banner->setContextLanguage($GLOBALS['localLanguage']);
	...
	if ($_POST['go'] == 'list') {
		Header("Location: ../../admin/?mcmd=1&module=CMS_BannerManager");
		exit;
	}
	...
	if ($_POST['go'] == 'edit') {
		...
		$link = urlPost($ignore);
		Header("Location: ../../admin/?mcmd=3&module=CMS_BannerManager&row_id[]=$new_id$link");
		exit;
	}
	
	Header("Location: ../../admin/?module=CMS_BannerManager&mcmd=1");
	exit;
