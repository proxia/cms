...
function preinit() {
	...
	var url = tinyMCE.getParam("external_anchor_list_url");
...
//	}
	...
	if (typeof(tinyMCEAnchorList) == "undefined" || tinyMCEAnchorList.length == 0){
	    for (var i=0; i<nodes.length; i++) {
	        if ((name = tinyMCE.getAttrib(nodes[i], "name")) != "")
	            html += '<option value="#' + name + '">' + name + '</option>';
	    }
	} else {
	    for (var i=0; i<tinyMCEAnchorList.length; i++) html += '<option value="#' + tinyMCEAnchorList[i] + '">' + tinyMCEAnchorList[i] + '</option>';
	}
