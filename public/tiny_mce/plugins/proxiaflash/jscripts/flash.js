...
function init() {
	...
	document.getElementById("filebrowsercontainer").innerHTML = getBrowserHTML('filebrowser','file','proxiaflash','proxiaflash');

	// Image list outsrc
	var html = getFlashListHTML('filebrowser','file','proxiaflash','proxiaflash');
	if (html == "")
	...
	var swfflashvars = '' + tinyMCE.getWindowArg('swfflashvars');
	// doplnit dalsie autostart, loop, flashvars
	...
	formObj.xflashvars.value = swfflashvars;
...
function insertFlash() {
	...
	var xflashvars      = formObj.xflashvars.value;
	...
	html += ''
		...
		+ 'border="0" alt="' + file + '" title="' + file + '" xflashvars="' + xflashvars + '" class="mceItemPROXIAFlash" />';
