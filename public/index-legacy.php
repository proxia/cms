	...
	if($u->isuserLogedIn() === false){
		...
		if ($_SESSION['user']['type'] == CMS_UserLogin::ADMIN_USER)
		{
		   $sites = $u->getAvailableSites();
			...
			$GLOBALS["smarty"]->assign("sites",$sites);
			$num_sites = 0;
...
//echo
		...
		if(CMS_ProjectConfig::getSingleton()->checkModuleAvailability('CMS_Catalog')){
			$module = CMS_Module::addModule('CMS_Catalog');
			$module->utilise();
		...
		if (($GLOBALS['proxiaType'] == 'catalog') && !($_GET['module']) && ($_GET['cmd']!=29) && ($_GET['cmd']!=30)){
			$_GET['module'] = 'CMS_Catalog';
			$_REQUEST['module'] = 'CMS_Catalog';
			$_GET['cmd'] = null;
		}
	...
	if ($GLOBALS['proxiaType'] == 'catalog'){
		...
		if(CMS_ProjectConfig::getSingleton()->checkModuleAvailability('CMS_Inquiry')){
			$module = CMS_Module::addModule('CMS_Inquiry');
			$module->utilise();
	...
	if(CMS_ProjectConfig::getSingleton()->checkModuleAvailability('CMS_EventCalendar')){
		...
		$GLOBALS["smarty"]->assign("current_link",$GLOBALS["current_link"]);
